...
def uart_int():
    ...
    for cmd in cmds:
        enter_uart1_test_menu()
        testlib.inputStr('uart_echo 1 115200 n 8 1 0' + '\r\n')
        testlib.sleep(100)
        enter_uart2_test_menu()
        paras = cmd.split()
        if paras[-1] == 'tx':
            testlib.runCase(cmd)
        else:
            if paras[-1] == '0':
                ...
                testlib.runCase(cmd, passlist=['character timeout int'])
            else:
                testlib.runCase(cmd, passlist=['receive data available int'])
